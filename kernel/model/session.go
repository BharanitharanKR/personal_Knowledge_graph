...
var (
	...
	BasicAuthHeaderValue = "Basic realm=\"Sedge Authorization Require\", charset=\"UTF-8\""
)
...
func CheckAuth(c *gin.Context) {
	...
	if !IsWorkspaceSessionAuthenticated(workspaceSession) {
		...
		if strings.HasPrefix(userAgentHeader, "Sedge/") || strings.HasPrefix(userAgentHeader, "Mozilla/") {
...
// 读操作和前端定时轮询（如 /api/ai/embeddingStat）不计入，避免 Sedge 开着却不操作时被判为活跃。
