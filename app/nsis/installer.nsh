...
Function AppendInstallLog
    ...
    ClearErrors
    FileOpen $7 "$TEMP\Sedge-install.log" a
...
!macro preInit
    ...
    ${IfNot} ${AtLeastWin10}
        ...
        MessageBox MB_ICONEXCLAMATION "非常抱歉，思源笔记无法在低于 Windows 10 的系统上进行安装$\n$\n\
            Sorry, Sedge cannot be installed on systems below Windows 10$\n"
        Quit
    ${EndIf}
    ...
    Push $R8
    Push $R7
    nsExec::Exec '"$SYSDIR\taskkill.exe" /F /IM "Sedge.exe"'
    Pop $R8
    nsExec::Exec '"$SYSDIR\taskkill.exe" /F /IM "Sedge-Kernel.exe"'
    Pop $R7
...
!macro customInstall
    ...
    RMDir /r "$PROFILE\AppData\Local\sedge-updater"
    nsExec::ExecToLog '"$SYSDIR\cmd.exe" /c mklink /H "$INSTDIR\resources\kernel\sedge.exe" "$INSTDIR\resources\kernel\Sedge-Kernel.exe" 2>nul || ver>nul'
    ${If} $installMode == "all"
...
!macro customUnInstall
    ${IfNot} ${isUpdated}
        IfFileExists "$PROFILE\.config\sedge\*.*" 0 skipConfigDelete
            MessageBox MB_YESNO "是否需要彻底删除全局配置（$PROFILE\.config\sedge\）？$\n$\n\
                Do you want to delete the global configuration ($PROFILE\.config\sedge\)?$\n" \
                /SD IDYES IDYES AcceptedRMConf IDNO SkippedRMConf
                AcceptedRMConf:
                    RMDir /r "$PROFILE\.config\sedge\"
                SkippedRMConf:
        skipConfigDelete:
    ${EndIf}
