...
if [[ "$TARGET" == 'amd64' || "$TARGET" == 'all' ]]; then
    ...
    export GOARCH=amd64
    setup_cc amd64
    go build -buildmode=pie -tags "fts5 sqlcipher" -o "../app/kernel-linux/Sedge-Kernel" -ldflags "-s -w -extldflags -static-pie" .
fi
if [[ "$TARGET" == 'arm64' || "$TARGET" == 'all' ]]; then
    ...
    export GOARCH=arm64
    setup_cc arm64
    go build -buildmode=pie -tags "fts5 sqlcipher" -o "../app/kernel-linux-arm64/Sedge-Kernel" -ldflags "-s -w -extldflags -static-pie" .
fi
