...
if defined BUILD_AMD64 (
    ...
    set GOARCH=amd64
    go build -tags "fts5 sqlcipher" -o "%PROJECT_ROOT%\app\kernel\Sedge-Kernel.exe" -ldflags "-s -w" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
)
if defined BUILD_ARM64 (
    ...
    go build -tags "fts5 sqlcipher" -o "%PROJECT_ROOT%\app\kernel-arm64\Sedge-Kernel.exe" -ldflags "-s -w" .
    if errorlevel 1 (
        exit /b %errorlevel%
    )
)
...
REM 打包完成后再建硬链接，避免将 sedge.exe 打进安装包
if defined BUILD_AMD64 (
    mklink /H "%PROJECT_ROOT%\app\kernel\sedge.exe" "%PROJECT_ROOT%\app\kernel\Sedge-Kernel.exe"
)
if defined BUILD_ARM64 (
    mklink /H "%PROJECT_ROOT%\app\kernel-arm64\sedge.exe" "%PROJECT_ROOT%\app\kernel-arm64\Sedge-Kernel.exe"
)
